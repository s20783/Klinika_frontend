...
function Schedule(props) {
    ...
    const scheduleList = props.schedules
    const start = props.start
    const end = props.end

    return (
        <>
            <h2 className="mt-6 w-full my-2 mb-6 text-2xl font-bold leading-tight text-center text-gray-700">
                {getFormattedDate1(start)}<span className="mx-3  "> - </span>{getFormattedDate1(end)}
            </h2>
            <table
                ...
                <tr className="shadow-xl rounded-lg">
                    ...
                    {scheduleList.filter(x => x.Dzien === 1).map(x => (
                        <td key={x.Data + x.IdWeterynarz} className="text-center w-full flex flex-wrap my-1">
                            <div className={(x.CzyZajete === false) ?
                                'border-l-4 border-green-400 w-full py-1'
                                : 'border-l-4 border-red-500 w-full py-1'}>
                                <div className="w-full">
                                    <span
                                        className={(x.Weterynarz === null) ? 'text-s' : 'text-s font-bold'}>
                            ...
                            <div className={(x.CzyZajete === false) ?
                                ...
                                <div className="w-full">
                                    <Link to={x.IdWizyta ? `/wizyty/editInfo/${x.IdWizyta}` : ''}>
                                            <span className={(x.Weterynarz === null) ? ' text-s ' : 'text-s font-bold'}>
                                                {dayjs(x.Data).format("HH:mm")} - {dayjs(x.Data).add(30, "minute").format("HH:mm")}
                                            </span>
                                    </Link>
                                </div>
