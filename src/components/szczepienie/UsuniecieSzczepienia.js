...
class UsuniecieSzczepienia extends React.Component {
    constructor(props) {
        super(props);
    ...
    removeSzczepienie = async (idSzczepienie) => {
        ...
        try {
            await deleteSzczepienie(idSzczepienie)
            ...
            <div class="bg-gray-200 flex items-center justify-center h-screen">
                <div
                    ...
                    <div class="modal-content py-9 px-5">
                        <p class="text-4xl mb-2 text-center font-bold">{t('szczepienie.deletingVaccination')}</p>
                        ...
                        <div class="flex justify-end pt-2">
                            ...
                            <button onClick={() => this.removeSzczepienie(idSzczepienie)}
                                    class="shadow-xl px-4 bg-blue-400 p-3 rounded-lg text-white hover:bg-blue-400">{t('szczepienie.deleteVaccination')}</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default withTranslation()(withRouter(withNavigate(UsuniecieSzczepienia)));