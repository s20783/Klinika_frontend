        ...
        <>
            <div className="flex items-center flex-row md:items-center justify-between py-4">
                ...
                <Link to="/dodajSzczepionke"
                    ...
                    <span className="hidden sm:inline">+ {t('szczepionka.button.addVaccine')}</span>
            ...
            <div className="overflow-x-auto shadow-lg sm:rounded-lg">
                <table className="w-full text-xs sm:text-sm md:text-base text-left text-gray-700 dark:text-gray-400">
                    ...
                    <tr>
                        <th scope="col" className="text-center px-1 md:px-6 py-3">{t('szczepionka.fields.name')}</th>
                        <th scope="col" className="text-center px-1 md:px-6 py-3">{t('szczepionka.fields.application')}</th>
                        <th scope="col" className="text-center px-1 md:px-6 py-3">{t('szczepionka.fields.mandatory')}</th>
                        <th scope="col" className="text-center px-1 md:px-6 py-3">{t('szczepionka.fields.periodOfValidity')}</th>
                        <th scope="col" className="text-center px-1 md:px-6 py-3">Producent</th>
                    ...
                    {list.map(x => (
                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600"
                            ...
                            <td className="text-center px-1 md:px-6 py-2">{x.OkresWaznosci}</td>
                            <td className="text-center px-1 md:px-6 py-2">{x.Producent ? x.Producent : "-"}</td>
                            <td className="text-center px-1 md:px-6 py-2">
                                <div className="flex justify-center">
                                    <TableItemEdit link={`/szczepionki/edycjaSzczepionki/${x.ID_lek}`}/>
                                    <TableItemDelete link={`/szczepionki/delete/${x.ID_lek}`}/>
                                </div>
                            </td>
                        </tr>
                    ))}
