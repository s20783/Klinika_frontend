...
function UslugaListTable(props) {
    const { t } = useTranslation();
    const list = props.uslugi
...
console.log(filteredData)
    ...
    <div>
            ...
            <table className="w-full text-sm text-left text-gray-700 dark:text-gray-400">
                ...
                <tr>
                    <th scope="col" className="px-6 uppercase py-3">{t('usluga.fields.name')}</th>
                    <th scope="col" className="px-6 uppercase py-3">{t('usluga.fields.description')}</th>
                    <th scope="col" className="px-6 uppercase py-3">{t('usluga.fields.painScale')}</th>
                    <th scope="col" className="px-6 uppercase py-3">{t('usluga.fields.narcosis')}</th>
                    <th scope="col" className="px-6 uppercase py-3">{t('usluga.fields.price')}</th>
                ...
                {filteredData.map(x => (
                    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600"
                        ...
                        <td scope="col" className="px-6 py-2">{x.NazwaUslugi} </td>
                        <td scope="col" className="px-6 py-2">{x.Opis}</td>
                        <td scope="col" className="px-6 py-2">{x.Dolegliwosc}</td>
                        <td scope="col" className="px-6 py-2">{x.Narkoza ? t("other.yes") : t("other.no")}</td>
                        <td scope="col" className="px-6 py-2">{x.Cena}</td>
                        <td scope="col" className="px-6 py-1">
                            <div className="list-actions">
                                <div className=" flex">
                                    <Link to={`/uslugi/edycjaUsluga/${x.ID_Usluga}`} className="list-actions-button-details flex-1">
                                    ...
                                    <Link to={`/uslugi/delete/${x.IdUsluga}`} className="list-actions-button-details flex-1">
...
export default UslugaListTable