...
class VisitServiceList extends React.Component {
    constructor(props) {
        super(props);
                        ...
                        <div className=" md:flex mb-6 mt-4 hidden ">
                            <div className="md:w-full">
                                <select name="IdUsluga" id="spec-content" onChange={this.handleChange}
                                    ...
                                    {
                                        uslugi.map(x => (
                                            <option selected={x.ID_Usluga === data.IdUsluga}
                                                    className={this.checkIfExist(uslugiWizyta, x.ID_Usluga) === true ? "text-gray-300" : ""}
                                                    value={x.ID_Usluga}> {x.NazwaUslugi}</option>
                                        ))}
                                </select>
...
export default withTranslation()(withNavigate(withRouter(VisitServiceList)));